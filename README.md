...
For example, `SASGameRecord_*.log` starts with the lifecycle marker, shared provenance rows and comparable run setup lines before adding compact turn snapshots (the captured values below are from an older run, with the current `recordRevision` field shown):
